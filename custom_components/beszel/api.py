...
from .const import API_SYSTEMS, API_STATS
...
class BeszelAPIClient:
    ...
    async def authenticate(self) -> bool:
        """Authenticate with the Beszel API."""
        try:
            auth_data = {
        ...
        try:
            ...
            async with self._session.get(
                ...
                if response.status == 401:
                    ...
                    raise BeszelAPIError(f"API request failed with status {response.status}")
    ...
    async def get_system_stats(self, system_id: str) -> dict[str, Any]:
        ...
        try:
            headers = {"Authorization": f"Bearer {self._auth_token}"}
            url = f"{self._base_url}{API_STATS.format(system_id=system_id)}"
    ...
    async def _get_system_stats_with_id(self, system_id: str, system_info: dict) -> dict[str, Any]:
