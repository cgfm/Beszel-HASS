...
def analyze_beszel_data(host: str, port: int, username: str, password: str, use_ssl: bool = False):
    ...
    response = session.post(f"{base_url}/api/collections/users/auth-with-password", json=auth_data)
    ...
    response = session.get(f"{base_url}/api/collections/systems/records", headers=headers)
    if response.status_code == 200:
        ...
        if systems:
            ...
            response = session.get(f"{base_url}/api/collections/systems/records/{system_id}", headers=headers)
            if response.status_code == 200:
            ...
            for collection in stats_collections:
                try:
                    response = session.get(f"{base_url}/api/collections/{collection}/records", headers=headers)
                    if response.status_code == 200:
                        data = response.json()
                        print(f"Collection '{collection}': {len(data.get('items', []))} items")
                        if data.get('items'):
                            print(f"  Sample item: {json.dumps(data['items'][0], indent=4)}")
                except:
                    pass
...
def main():
    """Main function."""
    if len(sys.argv) < 5:
        print("Usage: python analyze_beszel.py <host> <port> <username> <password> [use_ssl]")
        sys.exit(1)
